#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: BP_Computer

#include "Basic.hpp"

#include "BP_Computer_classes.hpp"
#include "BP_Computer_parameters.hpp"


namespace SDK
{

// Function BP_Computer.BP_Computer_C.ExecuteUbergraph_BP_Computer
// (Final, UbergraphFunction, HasDefaults)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void ABP_Computer_C::ExecuteUbergraph_BP_Computer(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "ExecuteUbergraph_BP_Computer");

	Params::BP_Computer_C_ExecuteUbergraph_BP_Computer Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_Computer.BP_Computer_C.MulticastHUBUnlock
// (Net, NetReliable, NetMulticast, BlueprintCallable, BlueprintEvent)

void ABP_Computer_C::MulticastHUBUnlock()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "MulticastHUBUnlock");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BP_Computer.BP_Computer_C.ToggleScreen
// (Net, NetReliable, NetClient, BlueprintCallable, BlueprintEvent)
// Parameters:
// bool                                    IsHidden                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor)
// bool                                    BypassPassword                                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor)

void ABP_Computer_C::ToggleScreen(bool IsHidden, bool BypassPassword)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "ToggleScreen");

	Params::BP_Computer_C_ToggleScreen Parms{};

	Parms.IsHidden = IsHidden;
	Parms.BypassPassword = BypassPassword;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_Computer.BP_Computer_C.OnVRPossess
// (Event, Public, BlueprintEvent)
// Parameters:
// bool                                    bPossess                                               (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor)

void ABP_Computer_C::OnVRPossess(bool bPossess)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "OnVRPossess");

	Params::BP_Computer_C_OnVRPossess Parms{};

	Parms.bPossess = bPossess;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_Computer.BP_Computer_C.ReceiveBeginPlay
// (Event, Protected, BlueprintEvent)

void ABP_Computer_C::ReceiveBeginPlay()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "ReceiveBeginPlay");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BP_Computer.BP_Computer_C.SetActivated
// (Net, NetReliable, NetServer, BlueprintCallable, BlueprintEvent)

void ABP_Computer_C::SetActivated()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "SetActivated");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BP_Computer.BP_Computer_C.InpAxisKeyEvt_MouseWheelAxis_K2Node_InputAxisKeyEvent_1
// (BlueprintEvent)
// Parameters:
// float                                   AxisValue                                              (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void ABP_Computer_C::InpAxisKeyEvt_MouseWheelAxis_K2Node_InputAxisKeyEvent_1(float AxisValue)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "InpAxisKeyEvt_MouseWheelAxis_K2Node_InputAxisKeyEvent_1");

	Params::BP_Computer_C_InpAxisKeyEvt_MouseWheelAxis_K2Node_InputAxisKeyEvent_1 Parms{};

	Parms.AxisValue = AxisValue;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_Computer.BP_Computer_C.UnPossess
// (Net, NetReliable, NetServer, BlueprintCallable, BlueprintEvent)

void ABP_Computer_C::UnPossess()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "UnPossess");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BP_Computer.BP_Computer_C.OnPossess
// (Event, Public, BlueprintEvent)

void ABP_Computer_C::OnPossess()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "OnPossess");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BP_Computer.BP_Computer_C.ReceivePossessed
// (Event, Public, BlueprintEvent)
// Parameters:
// class AController*                      NewController                                          (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void ABP_Computer_C::ReceivePossessed(class AController* NewController)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "ReceivePossessed");

	Params::BP_Computer_C_ReceivePossessed Parms{};

	Parms.NewController = NewController;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_Computer.BP_Computer_C.InpActEvt_Back_K2Node_InputActionEvent_0
// (BlueprintEvent)
// Parameters:
// const struct FKey&                      Key                                                    (BlueprintVisible, BlueprintReadOnly, Parm, HasGetValueTypeHash)

void ABP_Computer_C::InpActEvt_Back_K2Node_InputActionEvent_0(const struct FKey& Key)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "InpActEvt_Back_K2Node_InputActionEvent_0");

	Params::BP_Computer_C_InpActEvt_Back_K2Node_InputActionEvent_0 Parms{};

	Parms.Key = std::move(Key);

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_Computer.BP_Computer_C.InpActEvt_Use_K2Node_InputActionEvent_1
// (BlueprintEvent)
// Parameters:
// const struct FKey&                      Key                                                    (BlueprintVisible, BlueprintReadOnly, Parm, HasGetValueTypeHash)

void ABP_Computer_C::InpActEvt_Use_K2Node_InputActionEvent_1(const struct FKey& Key)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "InpActEvt_Use_K2Node_InputActionEvent_1");

	Params::BP_Computer_C_InpActEvt_Use_K2Node_InputActionEvent_1 Parms{};

	Parms.Key = std::move(Key);

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_Computer.BP_Computer_C.InpActEvt_Use_K2Node_InputActionEvent_2
// (BlueprintEvent)
// Parameters:
// const struct FKey&                      Key                                                    (BlueprintVisible, BlueprintReadOnly, Parm, HasGetValueTypeHash)

void ABP_Computer_C::InpActEvt_Use_K2Node_InputActionEvent_2(const struct FKey& Key)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "InpActEvt_Use_K2Node_InputActionEvent_2");

	Params::BP_Computer_C_InpActEvt_Use_K2Node_InputActionEvent_2 Parms{};

	Parms.Key = std::move(Key);

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_Computer.BP_Computer_C.InpActEvt_Interact_K2Node_InputActionEvent_3
// (BlueprintEvent)
// Parameters:
// const struct FKey&                      Key                                                    (BlueprintVisible, BlueprintReadOnly, Parm, HasGetValueTypeHash)

void ABP_Computer_C::InpActEvt_Interact_K2Node_InputActionEvent_3(const struct FKey& Key)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "InpActEvt_Interact_K2Node_InputActionEvent_3");

	Params::BP_Computer_C_InpActEvt_Interact_K2Node_InputActionEvent_3 Parms{};

	Parms.Key = std::move(Key);

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_Computer.BP_Computer_C.InpActEvt_AnyKey_K2Node_InputKeyEvent_0
// (BlueprintEvent)
// Parameters:
// const struct FKey&                      Key                                                    (BlueprintVisible, BlueprintReadOnly, Parm, HasGetValueTypeHash)

void ABP_Computer_C::InpActEvt_AnyKey_K2Node_InputKeyEvent_0(const struct FKey& Key)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "InpActEvt_AnyKey_K2Node_InputKeyEvent_0");

	Params::BP_Computer_C_InpActEvt_AnyKey_K2Node_InputKeyEvent_0 Parms{};

	Parms.Key = std::move(Key);

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_Computer.BP_Computer_C.InpActEvt_AnyKey_K2Node_InputKeyEvent_1
// (BlueprintEvent)
// Parameters:
// const struct FKey&                      Key                                                    (BlueprintVisible, BlueprintReadOnly, Parm, HasGetValueTypeHash)

void ABP_Computer_C::InpActEvt_AnyKey_K2Node_InputKeyEvent_1(const struct FKey& Key)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "InpActEvt_AnyKey_K2Node_InputKeyEvent_1");

	Params::BP_Computer_C_InpActEvt_AnyKey_K2Node_InputKeyEvent_1 Parms{};

	Parms.Key = std::move(Key);

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_Computer.BP_Computer_C.OnRep_UpdateLogin
// (BlueprintCallable, BlueprintEvent)

void ABP_Computer_C::OnRep_UpdateLogin()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "OnRep_UpdateLogin");

	UObject::ProcessEvent(Func, nullptr);
}


// Function BP_Computer.BP_Computer_C.ToggleWidgetInteraction
// (Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// bool                                    Enable                                                 (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor)

void ABP_Computer_C::ToggleWidgetInteraction(bool Enable)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "ToggleWidgetInteraction");

	Params::BP_Computer_C_ToggleWidgetInteraction Parms{};

	Parms.Enable = Enable;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_Computer.BP_Computer_C.Update Beam
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)

void ABP_Computer_C::Update_Beam()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Computer_C", "Update Beam");

	UObject::ProcessEvent(Func, nullptr);
}

}

